...
% Data Loading 
bone = readtable('VBCCA/modified_bone.xlsx');
...
x1_f_projected = W1' * x1_f;
x2_f_projected = W2' * x2_f;
initial_corr_f = corr(x1_f_projected', x2_f_projected');

x1_gc_projected = W1' * x1_gc;
x2_gc_projected = W2' * x2_gc;
initial_corr_gc = corr(x1_gc_projected', x2_gc_projected');

% change the ratio to difference 
% ratio_corr = initial_corr_f / initial_corr_gc;
diff_corr = initial_corr_f - initial_corr_gc;

disp('The correlation within space flight group is:');
disp(initial_corr_f);

disp('The correlation ground control group is:');
disp(initial_corr_gc);

disp('The ratio between two correlations is:');
disp(diff_corr);
...
subplot(2, 1, 1);
bar(W1_diff);
title('Difference of Bone Data Feature Weights (Space Flight - Ground Control)');
...
perm_corrs_diff = zeros(num_permutations, 1);
perm_corrs_f = zeros(num_permutations, 1);
perm_corrs_gc = zeros(num_permutations, 1);
...
for i = 1:num_permutations
    ...
    perm_corrs_f(i) = corr(perm_x1_f_projected(1, :)', perm_x2_f_projected(1, :)');
    perm_corrs_gc(i) = corr(perm_x1_gc_projected(1, :)', perm_x2_gc_projected(1, :)');
    perm_corrs_diff(i) = perm_corrs_f(i) - perm_corrs_gc(i);
end

% Calculate p-values for weight differences for each dimension
p_values_W1 = zeros(size(W1_diff));
p_values_W2 = zeros(size(W2_diff));
p_value_corr_f = mean(perm_corrs_f <= initial_corr_f);
p_value_corr_gc = mean(perm_corrs_gc >= initial_corr_gc);
p_value_diff = mean(perm_corrs_diff <= diff_corr);
...
subplot(3, 1, 1);
bar(p_values_W1);
title('P-values for Bone Data Feature Weights');
xlabel('Feature Index');
ylabel('P-value');
ylim([0 1]);
hold on;
...
figure;
subplot(3, 1, 1);
histogram(perm_corrs_f, 'Normalization', 'probability', 'FaceColor', '#FFC0CB', 'EdgeColor', 'black');
hold on;
xline(initial_corr_f, 'Color', 'blue', 'LineWidth', 2, 'LineStyle', '--');
xlabel('Correlation');
ylabel('Probability');
title(['Permutation Test for Space Flight Group, p=', num2str(p_value_corr_f)]);
hold off;

subplot(3, 1, 2);
histogram(perm_corrs_gc, 'Normalization', 'probability', 'FaceColor', '#FFC0CB', 'EdgeColor', 'black');
hold on;
xline(initial_corr_gc, 'Color', 'blue', 'LineWidth', 2, 'LineStyle', '--');
xlabel('Correlation');
ylabel('Probability');
title(['Permutation Test for Ground Control Group, p=', num2str(p_value_corr_gc)]);
hold off;

subplot(3, 1, 3);
histogram(perm_corrs_diff, 'Normalization', 'probability', 'FaceColor', '#FFC0CB', 'EdgeColor', 'black');
hold on;
xline(diff_corr, 'Color', 'blue', 'LineWidth', 2, 'LineStyle', '--');
xlabel('Correlation diff');
ylabel('Probability');
title(['Permutation Test for Correlation Difference, p=', num2str(p_value_diff)]);
hold off;
